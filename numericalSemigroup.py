...
class NumericalSemigroup: 
    ...
    def __init__(self, generators = []):
        ...
        if(len(self.gens) == 0):
            raise Execption("There must be at least one generator")
        try:
    ...
    def isSymmetric(self):
        if(gcd_list(self.gens) != 1):
            raise Exception("This property can only be tested for cofinite semigroups")
        return ((self.Frobenius%2 == 1) and (len(self.gaps) == ((self.Frobenius + 1)/2)))

    def isPsuedoSymmetric(self):
        if(gcd_list(self.gens) != 1):
            raise Exception("This property can only be tested for cofinite semigroups")
        return ((self.Frobenius%2 == 0) and (len(self.gaps) == ((self.Frobenius)/2)))

    def KunzCoordinates(self, m = None):
        if (m == None):
            m = self.multiplicity
        return [int((self.AperySet(m)[i] - i)/m) for i in range(1,len(self.AperySet(m)))] 
    
    def PseudoFrobeniusNumbers(self):
        newGens = self.minimalGenerators()
        PF = []
        for i in self.gaps:
            isPF = True
            for j in newGens:
